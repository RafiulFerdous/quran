...
function revelationType(api: string): "MECCAN" | "MEDINAN" {
  return api === "Medinan" ? "MEDINAN" : "MECCAN";
}

function sajdaType(sajda: ApiAyah["sajda"]): "NONE" | "RECOMMENDED" | "OBLIGATORY" {
  if (typeof sajda !== "object" || sajda === null) return "NONE";
  if (sajda.obligatory) return "OBLIGATORY";
  if (sajda.recommended) return "RECOMMENDED";
  return "NONE";
}
...
async function main() {
  ...
  const listPayload = await fetchJson<{ data: ApiSurahMeta[] }>(`${API}/surah`);
  ...
  for (const meta of listPayload.data) {
    ...
    const payload = await fetchJson<{
      data: Array<{ ayahs: ApiAyah[]; edition?: { identifier?: string } }>;
    }>(url);
