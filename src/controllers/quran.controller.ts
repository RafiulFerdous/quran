...
export async function listSurahs(req: Request, res: Response) {
...
export async function getAyatBySurah(req: Request, res: Response) {
  const raw = req.params.surahNumber;
  const surahNumber = Number.parseInt(
  ...
  const editionSlug =
    typeof req.query.edition === "string" && req.query.edition.length > 0
      ? req.query.edition
  ...
  const result = await quranService.getAyatForSurahPaginated(
    surahNumber,
    editionSlug,
    page,
    limit,
...
export async function searchAyat(req: Request, res: Response) {
  ...
  const result = await quranService.searchAyatByTranslation(
    q,
    editionSlug,
    page,
    limit,
