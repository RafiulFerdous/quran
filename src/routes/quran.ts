...
router.get("/surahs", async (req: Request, res: Response) => {
  ...
  if (total === 0) {
    return res.status(503).json({
      error: "no_data",
      message:
        "No Quran data in the database. Run: npm run db:seed",
    });
  }
...
router.get("/surahs/:surahNumber/ayat", async (req: Request, res: Response) => {
...
router.get("/ayat/search", async (req: Request, res: Response) => {
